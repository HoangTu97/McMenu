...
const apiSearchUrl = 'api/_search/nutrition-summaries';

// Actions

export const searchEntities = createAsyncThunk('nutritionSummary/search_entity', async ({ query, page, size, sort }: IQueryParams) => {
...
export const createEntity = createAsyncThunk(
  ...
  async (entity: INutritionSummary, thunkAPI) => {
    const result = await axios.post<INutritionSummary>(apiUrl, cleanEntity(entity));
...
export const updateEntity = createAsyncThunk(
  ...
  async (entity: INutritionSummary, thunkAPI) => {
    const result = await axios.put<INutritionSummary>(`${apiUrl}/${entity.id}`, cleanEntity(entity));
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
...
export const partialUpdateEntity = createAsyncThunk(
  ...
  async (entity: INutritionSummary, thunkAPI) => {
    const result = await axios.patch<INutritionSummary>(`${apiUrl}/${entity.id}`, cleanEntity(entity));
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
...
export const deleteEntity = createAsyncThunk(
  ...
  async (id: string | number, thunkAPI) => {
    const requestUrl = `${apiUrl}/${id}`;
    const result = await axios.delete<INutritionSummary>(requestUrl);
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
